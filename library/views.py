...
class ClearanceList(View):
    template_name = 'library/clearance-list.html'
    title = 'Details'
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        if len(reviews) > 0:
            for review in reviews:
                if review.reviewer.role == 'library':
        ...
        return render(request, 'library/clearance-details.html', context)


class ReviewClearance(View):
    template_name = 'library/review.html'
    title = 'Review'
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid() and clearance is not None and user is not None:
            ...
            clearance = Clearance.objects.get(id=pk)
            clearance.status = "Library Reviewed"            
            clearance.save()        
            ...
            send_mails(
                ...
                'Clearance Reviewed by Library',
                [student.user.email],
                {'user': user, 'clearance': clearance,
                    'review': review, 'student_name': student_name}
            )             
            return redirect('library_clearance_details_view', pk)
        context = {
            'title': self.title,
            'form': form
        }

...
class ReviewClearance(View):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid() and clearance is not None and user is not None:
            ...
            clearance = Clearance.objects.get(id=pk)
            clearance.status = "Dean of Students Reviewed"            
            clearance.save()  
            ...
            send_mails(
                ...
                'Clearance Reviewed by Dean of Students',
                [student.user.email],
                {'user': user, 'clearance': clearance,
                    'review': review, 'student_name': student_name}
            )                   
            return redirect('dos_clearance_details_view', pk)
        context = {
            'title': self.title,
            'form': form
        }

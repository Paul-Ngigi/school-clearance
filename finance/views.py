...
class ClearanceList(View):
    template_name = 'finance/clearance-list.html'
    title = 'Details'
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        if len(reviews) > 0:
            for review in reviews:
                if review.reviewer.role == 'finance':
        ...
        return render(request, 'finance/clearance-details.html', context)


class ReviewClearance(View):
    template_name = 'finance/review.html'
    title = 'Review'
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid() and clearance is not None and user is not None:
            ...
            clearance = Clearance.objects.get(id=pk)
            clearance.status = "Finance Reviewed"            
            clearance.save()     
            ...
            send_mails(
                ...
                'Clearance Reviewed by Finance',
                [student.user.email],
                {'user': user, 'clearance': clearance,
                    'review': review, 'student_name': student_name}
            )                
            return redirect('finance_clearance_details_view', pk)
        context = {
            'title': self.title,
            'form': form
        }
